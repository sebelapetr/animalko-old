	...
	<section>
      ...
      <div class="col-sm-3">                              
        <div class="col-sm-12" style="padding:0;margin-bottom:15px;border:1px solid #0003">
                <?php     
                $query_length = $mysqli->query("select jmeno_kategorie, nazev from kategorie where skupina = 2");  
                echo '<div class="col-sm-12" style="background:#f3f3f3">    
                        <a href="?p=Chovatelske-vybaveni">
                              <h3 style="color:black;text-decoration:;font-weight:400;font-size:18px;margin:15px 0;">Chovatelské vybavení</i></h3>   
                           </a>  
                      </div>';        
                      $string_kata="";            
        ...
        <?php    
                ...
                $query_length = $mysqli->query("select distinct zakoupen,jmeno_znacka from produkt where kategorie like 'Chovatelske%' ".$string_kata." and aktivni = 1 order by zakoupen desc limit 5");  
                while ($row_length = $query_length->fetch_assoc()) { 
      ...
      <div class="col-sm-9">     
       ...
       <div class="col-sm-3" style="margin-top:15px;padding-left:0px;padding-right:0px;"> 
          ...
          <?php
            ...
            echo '<a href="?p=Chovatelske-vybaveni" style="color:black;text-decoration:none;font-weight:300;">Chovatelské vybavení</a>';             
          ?>
          </div>       
                ...
                $number=0;
                $p=0;
                $query_length = $mysqli->query("select nazev,jmeno_kategorie from kategorie where skupina = 2");  
                ...
                */  
                $row_nadpis['nazev']="Chovatelské vybavení";   
                $row_nadpis['jmeno_kategorie']= "Chovatelske-vybaveni";            
                ...
                //echo "SELECT id_produkt, katalog_cena_DPH, obrazek, jmeno_produkt, url from produkt where kategorie in ($string) LIMIT $limit OFFSET $offset<br>";    
                  ...
                  <form method="post" action="?p='.$page.'?action=add&id='.$row3["id_produkt"].'" target="formDestination">      
                      ...
                      <a href="?p='.html_entity_decode($row3['url']).'">
                   ...
                   echo'
                      ...
                      <hr style="margin:8px;">
        											<p>
                              <a href="?p='.html_entity_decode($row3['url']).'">'.html_entity_decode($row3['jmeno_produkt']).'</a>
